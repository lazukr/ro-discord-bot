...
class AutoMarketTask extends Task {
  static loggedIn = sessionConfig.cookie ? true : false;
  ...
  async process(channel, feed = null) {
    ...
    if (!this.name) {
      this.name = automarket.name;
    }

    if (automarket.error === nvro.ERROR.NO_LOGIN) {
      logger.info(`automarket failed to login`);

      if (!AutoMarketTask.messageSent) {
        channel.send(`Bear was unable to login. Please fill out Captcha <@${config.ownerid}>.`);
        AutoMarketTask.messageSent = true;
        AutoMarketTask.loggedIn = false;
      }
      return;
    }
    ...
    if (automarket.error !== nvro.ERROR.NONE) {
      ...
      logger.info(`automarket error on ${automarket.id} - ${automarket.name}: ${automarket.error}`);
      this.results = '';
...
class TaskFactory {
  ...
  _processMessageArgs(props) {
    ...
    const inIndex = cmd.lastIndexOf('in');
    const everyIndex = cmd.lastIndexOf('every');

    if (!(inIndex > -1 || everyIndex > -1)) {
      return null;
    }

    const match = inIndex > -1 ? inIndex :
      everyIndex;
    
    const remind = everyIndex > -1 ? everyRemind(cmd.substring(everyMatch + 1)) :
      inIndex > -1 ? inDurationDatetime(cmd.substring(inIndex + 1)) :
      null;
    ...
    let msg = cmd.substring(0, match).trim();
    if (msg[msg.length - 1] == ",") {
...
module.exports.AutoMarketTask = AutoMarketTask;