...
class AutoMarketTask extends Task {
  ...
  async process(channel) {
    logger.info(`Processing task in ch: ${channel}. Task: ${this}`);
    const itemID = this.itemID;
    const automarket = await nvro.getLiveMarketData(itemID);
    logger.info(this);
    ...
    if (prettyTable.hasResults(this.filters)) {

      // using ping only on change
      //
      /*
      logger.info(`Posting filtered results...`);
      const page = prettyTable.getPage(1, this.filters);
      channel.send(`<@${this.ownerid}>! Bear has message for you.\n${page}`);
      return;
      */

      // potential use for the future where it only pings on change.

      
      logger.info(`Automarket has valid filtered results, checking with previous`);
      const page = prettyTable.contents;
      // only ping if there were no results 
      // or if results do not match current results
      if (!this.results || 
        JSON.stringify(this.results) !== JSON.stringify(page)) {
        const diff = this.results ? this.results
          .filter(x => !page.includes(x))
          .concat(page.filter(x => !this.results.includes(x))) :
          `All new`;
        logger.info(`New results available... listing difference...`);
        logger.info(diff);
    ...
    logger.info(`No results after filters`);
  }
}
