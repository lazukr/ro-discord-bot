...
class DataTable {
  ...
  static rowify($, table, by = "html") {
    const data = this._rowify($, table, by);
    return new DataTable(data.header, data.contents); 
  }

  static _rowify($, table, by) {
...
class MarketTable extends DataTable {
  ...
  abbreviate(th, abbrev) {
    Object.keys(abbrev).forEach(ab => {
      if (this.header[th[ab]]) {
        this.header[th[ab]] = abbrev[ab];  
      }
    }); 
  }

  locToNavi(th, loc_navi) {
    this.contents.forEach(tr => {
      const locArray = tr[th.LOCATION].split(',');
      tr[th.LOCATION] = locArray[0] == NOVA_VEND ?
