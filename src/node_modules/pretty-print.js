...
const TBL_LIM = 1900;
const ENTRY_LIM = [100, 50, 25, 20, 15, 10, 5, 1];

class PrettyTableFactory {
  constructor(table) {
    if (table.table instanceof dt.MarketTable || table.table instanceof dt.NewMarketTable) {
...
class PrettyTable {
  ...
  _stringify(separators, header, contents) {
    ...
    table = table.map(tr => {
      ...
      //tr[keys[keys.length - 1]] = tr[keys[keys.length - 1]].trim();
      return Object.values(tr).join(' | ').trim();
    }); 
  ...
  _paginate(maxLength, contents) {
    ...
    if (this.name && this.id) {
      header += this.id.toString().length + 3 + this.name.length;
    }

    // account for the entries text itself
    const entrycount = 40; 
  
    const rowLength = maxLength.length;
    const tableLength = contents.length; 

    let i = 0;
    while ((ENTRY_LIM[i] + 2) * rowLength + entrycount + header > TBL_LIM) {
      //console.log(ENTRY_LIM[i] * rowLength + entrycount + header);
      i++;
    }
    /*
    logger.info(`header length: ${header}`);
    logger.info(`number of rows: ${ENTRY_LIM[i]}`);
    logger.info(`length of row: ${rowLength}`);
    logger.info(`entry count length: ${entrycount}`);
    logger.info(`total msg length: ${ENTRY_LIM[i]} * ${rowLength} + ${entrycount} + ${header} = ${ENTRY_LIM[i] * rowLength + entrycount + header}`);
    */
    return { 
  ...
  getPage(page, filters = {}, supressEntries = false) { 
    ...
    const paginate = this._paginate(stringify.separators, stringify.contents);
...
class PrettyMarketTable extends PrettyTable {
  ...
  _prettyTableStringify(separators, header, contents) {
    return this._stringify(separators, header, contents); 
  } 
  ...
  getPage(page, filters, supressedEntries = false) {
    ...
    const tempTable = this._doFilters(filters);
    const table = tempTable.map(obj => {
      const addprop = "Additional Properties";
      if (obj[addprop]) {
        obj[addprop] = obj[addprop].replace(/\s?Modification Orb/g, "").trim();
      }
