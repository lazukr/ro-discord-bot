...
class PrettyTable {
  ...
  _paginate(maxLength, contents) {
    ...
    const rowLength = maxLength.length + 4;
  ...
  getPage(page, filters = {}, supressEntries = false) { 
    ...
    const paginate = this._paginate(maxLength, stringify.contents);
...
class PrettyMarketTable extends PrettyTable {
  ...
  getPage(page, filters, supressedEntries = false) {
    ...
    const table = tempTable.map(obj => {
      obj["Additional Properties"] = obj["Additional Properties"].replace(/\s?Modification Orb/g, "").trim();
      return obj;
    });
