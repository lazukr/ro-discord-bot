...
class PrettyMarketTable extends PrettyTable {
  ...
  _processAddProps(contents, addprops) {
    ...
    const filtered = contents.filter(tr => {
      ...
      const filtered = addprops.every(props => trprops.includes(props.toLowerCase()));
      return filtered;  
    });
    ...
    const secondfiltered = filtered.filter(tr => {
      const trprops = tr[HEADERS.ADDPROPS].toLowerCase().split(',').map(x => x.trim());
      
      console.log(trprops);  
      let count = 0;
      for (let i = 0; i < addprops.length; i++) {
        for (let j = 0; j < trprops.length; j++) {
          if (trprops[j].includes(addprops[i])) {
            trprops.splice(j, 1);
            count++;
            break;
          }
