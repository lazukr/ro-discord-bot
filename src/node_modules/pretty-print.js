...
class PrettyTableFactory {
  constructor(table) {
    if (table.table instanceof dt.MarketTable) {
      ...
      return new PrettyMarketTable(table.table, table.id, table.name);
    }
    logger.debug("data-table");
    return new PrettyTable(table.table);
  }
};
...
class PrettyTable {
  ...
  getPage(page, filters = {}) { 
    ...
    const contents = stringify.contents;
    const begEntry = 1 + paginate.entryPerPage * (page - 1);
    const endEntry = paginate.entryPerPage * page;
    const components = {
      header: header,
...
class PrettyMarketTable {
  constructor(table, id, name) {
    this.id = id;
    this.name = name;
    this.header = table.header;
    this.contents = table.contents; 
