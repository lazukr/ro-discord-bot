...
async function getCharaLink(message, args, max_values, link) { 
  ...
  const params1 = values.params1 > INVALID ? values.params1 : getRandInt(0, max_values[0]);
  const params2 = values.params2 > INVALID ? values.params2 : getRandInt(0, max_values[1]);
  ...
  const msg = await message.channel.send(`\`${charaName} - ${params1}/${params2}\``, {
    files: [`${link}${charaName}/${params1}/${params2}?${timestamp}.png`,
    ]
  }); 
...
function checkValues(string, max_values) {
  ...
  if (params.length > 1) {
    params1 = !isNaN(params[0]) && inBetween(params[0], 0, max_values[0]) ? params[0] : params1;
    params2 = !isNaN(params[1]) && inBetween(params[1], 0, max_values[1]) ? params[1] : params2;
  }
