...
async function getPopulation() {
  const $ = await sc.getPage(MARKET_URL, {});
...
async function getLiveMarketData(itemID, login = 0) {
  ...
  if (loginBtn === 'Log In') {
    // we've already attempted log in, but it did not work
  ...
  if (loginBtn === 'Log In' && !login) {
    // we haven't logged in yet. Thus, log in.
    logger.log(`Not logged in. Logging into Nova...`);
    await sc.login();
    return getLiveMarketData(itemID, 1);
  }
  */
