...
class Scheduler {
  ...
  getScheduledJobs() {
    ...
    logger.info(`Going through all items in scheduler...`);  
    ids.forEach(id => {
      const name = this.scheduledJobs[id] ? this.scheduledJobs[id].name : "None";
  ...
  async processTask(jobName, taskid, task) {
    logger.info(`Processing task... ${jobName} ${taskid}. Task: ${JSON.stringify(task)}`);
