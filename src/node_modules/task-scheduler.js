...
//      scheduled: "Interval (mins)",
  ...
  async _initialTaskHandler() {
    ...
    this.scheduler.scheduleJob('interval', '30 */1 * * * *', async () => {
  ...
  async _processAutomarkets() {
    ...
    .map(entry => {
      ...
      //console.log(data);
      result.table.intToStrCols(nvro.HEADERS.QTY);
      result.table.intToStrCols(nvro.HEADERS.PRICE);
      result.table.intToStrCols(nvro.HEADERS.REFINE);
      const prettyTable = new pp.PrettyTableFactory(result);
      const newResults = prettyTable.hasResults(data.filters) ?
