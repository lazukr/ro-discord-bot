...
class Scheduler {
  ...
  _cancelJob(key) {
    if (key in this.scheduledJobs &&
      ...
      logger.info(`Scheduled jobs: ${this.scheduledJobs}`);
    }
  }
  ...
  async getMessageList(page) {
    ...
    const list = await Promise.all(msgList.map(async (data, i) => {
      ...
      const scheduled = isNaN(data.scheduled) &&
        ...
        data.scheduled :
      await bot.scheduler
        .timeLocalize(data.ownerid, data.scheduled);
      return {
        index: (i + 1).toString(),
        owner: data.owner,
        message: data.args,
  ...
  async processTask(taskid, task) {
    logger.info(`Processing task... ${taskid}. Task: ${task}`);
    const channel = this.client.channels.get(task.channel);
    await task.process(channel);
    
    if (task.type == taskFactory.MSG) {
      const parsedDate = Date.parse(task.scheduled);
      if (isNaN(task.scheduled) && !isNaN(parsedDate)) {
        return;
      }
  ...
  async queue(taskid, task) {
    ...
    const job = this.scheduler.scheduleJob(task.scheduled, async function() {
