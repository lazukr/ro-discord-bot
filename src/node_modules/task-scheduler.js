...
//      scheduled: "Interval (mins)",
  ...
  async getScheduledAutomarketJobs(ownerid = null) {
    logger.info(`Getting automarket items in scheduler...`);
    const list = await this._getMessageOfType(taskFactory.AUTOMARKET);
    return list.filter(job => ownerid ? job.ownerid === ownerid : true);
  }
  ...
  async _getAutoMarketList(ownerid = null) {
    const list = this.getScheduledAutomarketJobs(ownerid);
    if (list.length === 0) {
      return;
    }
  ...
  async getAutoMarketList(page, ownerid = null) {
    const automarketList = await this.getScheduledAutomarketJobs(ownerid);
  ...
  async processTask(jobName, taskid, task) {
    ...
    if (task.type == taskFactory.AUTOMARKET 
      && !taskFactory.AutoMarketTask.loggedIn) {
        if (!taskFactory.AutoMarketTask.messageSent) {
          taskFactory.AutoMarketTask.messageSent = true;
          const ownerChannel = this.client.channels.get(config.ownerChannel);
          ownerChannel.send(`<@${config.ownerid}>! Bot could not login. Please fill out session!`);
        }
        return;
    }
