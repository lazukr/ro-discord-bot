...
async function hit(link) {
  try {
    const { body, request } = await hooman.get(link);
    return request.options.headers.cookie;
  } catch (e) {
    logger.error(`Could not get the link ${link}. ${e}`);
    }
}

async function login(captcha = null) {
  const loginCookie = await hit(LINK);
  if (!loginCookie) {
    return 0;
  }
  ...
  try {
    ...
    try {
      ...
      logger.info(`Cookie: ${sessionConfig.cookie}`);
      fs.writeFileSync('session.json', JSON.stringify(sessionConfig));
      tf.AutoMarketTask.cookie = sessionConfig.cookie;
...
async function getPage(url, qs = {}) {
  logger.info(tf.AutoMarketTask.cookie);

  const options = {
    searchParams: qs,
    headers: {
      cookie: tf.AutoMarketTask.cookie,
    },
  }
