...
exports.run = async (discordBot, message, args) => {
  ...
  const bgVal = bgPose.bg > INVALID ? bgPose.bg : getRandInt(0, BG_MAX); // check if valid bg, else random
  const poseVal = bgPose.pose > INVALID ? bgPose.pose : getRandInt(0, POSE_MAX); // check if valid pose, else random
  if (bgPose.bg > INVALID || bgPose.pose > INVALID) { // if either bg or pose is valid, ignore last arg for character name
    args.pop();
  }
